-- Fix: gen_random_bytes() lives in the "extensions" schema (pgcrypto),
-- but the RPC called it unqualified. With search_path = 'public' only,
-- the function was not found at call time ("function gen_random_bytes(integer)
-- does not exist"). Schema-qualify the call so it resolves regardless of
-- search_path, matching the existing extensions.digest() fix.
